...
export default function Dev() {
      ...
      if (progettiError) console.error("Errore progetti:", progettiError);
        ...
        if (!isAdmin && userId !== null) {
          const filtered = (progettiData || []).filter(p =>
            (p.developer_ids && p.developer_ids.includes(userId)) || p.developer_id === userId
          );
  ...
  const getAssignedDevelopers = (project: Progetto) => {
    const ids = project.developer_ids?.length ? project.developer_ids : project.developer_id ? [project.developer_id] : [];
  ...
  const handleAssign = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from("Project")
      .update({ developer_ids: developerIds })
  ...
  const handleLogSave = async (e: React.FormEvent) => {
    ...
    const { error: updateErr } = await supabase
      .from("Project")
      .update({ dev_remaining_h: newRemaining })
    ...
    const { error: insertErr } = await supabase
      .from("TimeLogs")
      .insert([{ person_id: user?.id || null, project_id: currentLogProject.id, type: "Dev", hours, note: logForm.note }]);

    if (insertErr) {
      await supabase
        .from("Project")
        .update({ dev_remaining_h: currentLogProject.dev_remaining_h ?? initialEffort })
        .eq("id", currentLogProject.id);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold">Copertura Dev</h1>
          <p className="text-gray-500 text-sm">Tabella dei progetti Dev con release madre, stato e effort.</p>
        </div>
      </div>
          ...
          <table className="min-w-[900px] divide-y divide-gray-200 text-left">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Dev assegnati</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {progetti.map((progetto) => {
                ...
                const devEffort = progetto.dev_effort_h ?? (progetto.dev_effort ? progetto.dev_effort * 8 : 0);
                const devRemaining = progetto.dev_remaining_h ?? devEffort;
                ...
                return (
                  <tr key={progetto.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">
                      {getAssignedDevelopers(progetto).length > 0 ? (
                        <div className="flex flex-wrap gap-2">
                          {getAssignedDevelopers(progetto).map((person) => (
                        ...
                        <span className="text-amber-500 text-xs italic">Nessun dev assegnato</span>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{formatHoursValue(devEffort)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{formatHoursValue(devRemaining)}</td>
      ...
      {isAssignOpen && currentAssignProject && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 m-4 relative border">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Assegna sviluppatore a {currentAssignProject.name}</h2>
            <form onSubmit={handleAssign} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-600 uppercase mb-1">Sviluppatori</label>
                <select
                  multiple
      ...
      {isLogOpen && currentLogProject && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 m-4 relative border">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Registra ore Dev per {currentLogProject.name}</h2>
