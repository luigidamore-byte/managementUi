...
export default function Bugfix() {
  ...
  const getAssignedDevelopers = (project: Progetto) => {
    const ids = project.bugfix_developer_ids?.length ? project.bugfix_developer_ids : [];
  ...
  const handleAssign = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from("Project")
      .update({ bugfix_developer_ids: developerIds })
  ...
  const handleLogSave = async (e: React.FormEvent) => {
    ...
    const { error: updateErr } = await supabase
      .from("Project")
      .update({ bugfix_remaining_h: newRemaining })
    ...
    if (insertErr) {
      await supabase
        .from("Project")
        .update({ bugfix_remaining_h: currentLogProject.bugfix_remaining_h ?? initialEffort })
        .eq("id", currentLogProject.id);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold">Copertura Bugfix</h1>
          <p className="text-gray-500 text-sm">Tabella dei progetti Bugfix con release madre, stato e effort.</p>
        </div>
      </div>
          ...
          <table className="min-w-[900px] divide-y divide-gray-200 text-left">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Bugfix assegnati</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {progetti.map((progetto) => {
                ...
                const bugfixEffort = progetto.bugfix_effort_h ?? (progetto.bugfix_effort ? progetto.bugfix_effort * 8 : 0);
                const bugfixRemaining = progetto.bugfix_remaining_h ?? bugfixEffort;
                ...
                return (
                  <tr key={progetto.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{formatHoursValue(bugfixEffort)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{formatHoursValue(bugfixRemaining)}</td>
      ...
      {isAssignOpen && currentAssignProject && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 m-4 relative border">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Assegna sviluppatori Bugfix a {currentAssignProject.name}</h2>
      ...
      {isLogOpen && currentLogProject && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 m-4 relative border">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Registra ore Bugfix per {currentLogProject.name}</h2>
